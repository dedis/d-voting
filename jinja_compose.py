...
parser.add_argument('-n', '--number', metavar='NUM_NODE', type=int,
                    default=3,
                    help='Specify number of d voting nodes')
...
if args.template is not None:
    ...
    content = Template(args.template.read()).render(N_NODE=n_node)
...
if not args.generate:
    import subprocess
    run_list = ['docker-compose', '-f', args.output.name] + extras
    try:
        subprocess.run(run_list)
    except KeyboardInterrupt:
        print('Interrupted')
        subprocess.run(['rm', '-rf', './nodedata'])
        subprocess.run(['mkdir', 'nodedata'])
