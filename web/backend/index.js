...
app.get('/api/getTkKey', (req, res) => {
  ...
  axios
    ...
    .then(response => {
      ...
      const url =
        'https://tequila.epfl.ch/cgi-bin/tequila/requestauth?requestkey=' + key
      res.json({ url: url })
    })
    .catch(error => {
      console.error(error)
...
app.get('/api/get_user_rights', (req, res) => {
  ...
  if (sciper) {
    ...
    if (user['role'] === 'admin') {
      var users = []
      usersDB.getRange().forEach(({ key, value }) => {
        users.push(value)
      })
    } else {
...
app.post('/api/add_role', (req, res) => {
  if (req.session.userid) {
    ...
    if (requester['role'] === 'admin') {
      ...
      return usersDB.put(sciper, user).catch(error => {
