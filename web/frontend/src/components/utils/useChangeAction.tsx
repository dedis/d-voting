...
const useChangeAction = (
  ...
  const modalClose = (
    <ConfirmModal
      ...
      textModal={Translations[context].confirmCloseElection}
  ...
  const modalCancel = (
    <ConfirmModal
      ...
      textModal={Translations[context].confirmCancelElection}
      setUserConfirmedAction={setUserConfirmedCanceling}
    />
  );
  const [postError, setPostError] = useState(
    Translations[context].operationFailure
  );
    ...
    switch (status) {
        ...
        return (
          <span>
            <button
              ...
              onClick={handleClose}
            >
              {Translations[context].close}
            </button>
            <button className="election-btn" onClick={handleCancel}>
              {Translations[context].cancel}
            </button>
          </span>
        );
      case CLOSED:
        return (
          <span>
            {isShuffling ? (
              <p className="loading">{Translations[context].shuffleOnGoing}</p>
            ) : (
              <span>
                <button className="election-btn" onClick={handleShuffle}>
                  {Translations[context].shuffle}
                </button>
              </span>
            )}
          </span>
      ...
      case SHUFFLED_BALLOT:
        return (
          <span>
            {isDecrypting ? (
              <p className="loading">{Translations[context].decryptOnGoing}</p>
            ) : (
              <span>
                <button className="election-btn" onClick={handleDecrypt}>
                  {Translations[context].decrypt}
                </button>
              </span>
            )}
          </span>
          ...
          <span>
            ...
            >
              <button className="election-btn">
                {Translations[context].seeResult}
              </button>
            </Link>
          </span>
        );
