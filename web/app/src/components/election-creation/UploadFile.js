...
const UploadFile = ({ setShowModal, setTextModal }) => {
  ...
  return (
    <div className="form-content-right bg-gray-200 flex-1 m-1 p-10">
      ...
      <input type="file" className='block bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white text-xs py-1 px-4 border border-blue-500 hover:border-transparent rounded'
        value={name}
        multiple={false}
        accept='.json'
      ...
      <input type="button" className='block bg-blue-500 hover:bg-blue-700 text-white font-bold mt-7 py-2 px-5 rounded-full text-xs' value={Translations[context].createElec} onClick={uploadJSON} />
    </div>
  );
}
